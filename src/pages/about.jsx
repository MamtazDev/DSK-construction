...
const About = () => {
  ...
  const accessToken = "IGQWRQTUZAsZAm40REs4VU9SdEMtT3ZApSEtQY2pxajQ2cmhabkQ1SmtsaFg5NWJSbjRDTnh4QlhieXVRUU0tQkpieEFRWVc2NUMtcWZAMeWlSUkhQN2dhYjZAYZAlVjdHBDXzA4NDByVklEMGNsbEZABRWg1aXNlZAkFCNFEZD";
  ...
  useEffect(() => {
    fetch(mediaUrl)
      ...
      .then(async (mediaData) => {
        if (mediaData.data) {
          const fetchedPosts = mediaData.data;

          // Fetch comments for each post
          const postsWithComments = await Promise.all(
            fetchedPosts.map(async (post) => {
              const commentsUrl = `https://graph.instagram.com/v13.0/${post.id}/comments?access_token=${accessToken}`;
              const commentsResponse = await fetch(commentsUrl);
              const commentsData = await commentsResponse.json();
              const postWithComments = { ...post, comments: commentsData.data };
  ...
  return (
    <>
      ...
      <blockquote className="instagram-media " data-instgrm-permalink="https://www.instagram.com/dsk.construction/?utm_source=ig_embed&amp;utm_campaign=loading" data-instgrm-version="14" style={{ background: '#FFF', border: '0', borderRadius: '3px', boxShadow: '0 0 1px 0 rgba(0,0,0,0.5),0 1px 10px 0 rgba(0,0,0,0.15)', margin: '1px', maxWidth: '100%', minWidth: '326px', padding: '0', width: '99.375%', WebkitWidth: 'calc(100% - 2px)', width: 'calc(100% - 2px)' }}>
        <div style={{ padding: '16px',margin:"auto" }}>
          <a href="https://www.instagram.com/dsk.construction/?utm_source=ig_embed&amp;utm_campaign=loading" style={{ background: '#FFFFFF', lineHeight: '0', padding: '0 0', textAlign: 'center', textDecoration: 'none', width: '100%' }} target="_blank">
          ...
          <p style={{ color: '#c9c8cd', fontFamily: 'Arial, sans-serif', fontSize: '14px', lineHeight: '17px', marginBottom: '0', marginTop: '8px', overflow: 'hidden', padding: '8px 0 7px', textAlign: 'center', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            <a href="https://www.instagram.com/dsk.construction/?utm_source=ig_embed&amp;utm_campaign=loading" style={{ color: '#c9c8cd', fontFamily: 'Arial, sans-serif', fontSize: '14px', fontStyle: 'normal', fontWeight: 'normal', lineHeight: '17px' }} target="_blank">DSK CONSTRUCTION CORP</a> (@<a href="https://www.instagram.com/dsk.construction/?utm_source=ig_embed&amp;utm_campaign=loading" style={{ color: '#c9c8cd', fontFamily: 'Arial, sans-serif', fontSize: '14px', fontStyle: 'normal', fontWeight: 'normal', lineHeight: '17px' }} target="_blank">dsk.construction</a>) • Instagram photos and videos
          </p>
        </div>
      </blockquote>
